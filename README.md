...
236 题自建评测集 + BIRD 公开基准 + 真实数据集 Olist；另有 53 题行为评测，测"该问的问、该查的查"；区间按题目聚类校正，改动在同一批题上按题配对检验
...
schema 装得下就完整交给模型；装不下时渐进式披露：先给表目录（体积约为完整定义的 11–15%），模型选表后再展开定义、按需查列的真实取值；业务口径、相似例句和用户记忆按需注入
...
步骤与回答逐字流式推送；答案下方附口径说明（从 SQL 语法树生成的筛选、分组、排序、条数，不经过模型），能展开到 SQL、原始结果和本次注入的上下文
...
同一会话里可以接着追问（"那按州呢"），前几轮的问题和 SQL 会带进上下文；保存的口径和偏好注入之后的每次提问，定义过的说法不再反问，公网部署时每个访客的记忆相互隔离
...
"为什么 3 月销售额涨了"这类问题拆成几步查询并行执行，看完结果决定是否下钻；结论每句标注出自第几步，逐句核对数字确实来自所引用的那一步
...
每个回答可以点 👍/👎；运行记录同时是审计日志，差评连同问题、上下文和 SQL 导出成待标注的评测用例，补上标准答案就进回归集
...
可选开启：多采样一条 SQL，两条执行结果一致就采用，不一致再补采样、按执行结果多数决；只在有分歧时多花钱
...
```mermaid
flowchart LR
    Q([提问]) --> CTX["上下文组装<br/>schema 直供或渐进式披露<br/>+ 业务口径 / 例句 / 记忆"]
    CTX --> GEN[生成 SQL]
    GEN -- 口径不明或缺数据 --> ASK(["向用户确认<br/>给出可选口径"])
    ASK -. 用户选择后重新提问 .-> Q
    GEN -- 问口径或表结构 --> META["依据表结构回答<br/>标明未查询数据"]
    META --> OUT
...
深度分析模式在这条管线外面再套一层：模型先把问题拆成几个子问题，每个子问题并行走上面的完整流程（守卫、修复、取值核对都在），
看完结果决定是否追加下钻，最后写结论——每句标注出自第几步，逐句核对数字来自所引用的那一步，对不上就只列出各步结果。

```mermaid
flowchart LR
    Q2([分析问题]) --> PLAN[拆成 2–4 个子问题]
    PLAN --> S1[子查询 1] & S2[子查询 2] & S3[子查询 …]
    S1 & S2 & S3 --> REV{够了吗}
    REV -- 还要下钻 --> S4[追加子查询] --> REP
    REV -- 够了 --> REP["写结论<br/>逐句核对出处"]
```
...
| 模型生成的画图代码 | 不在主进程执行：有 Docker 时用断网容器（内存 / CPU / 进程数限额），否则用带资源限额、换成无权限用户的子进程，读不到 API Key；产物只接受普通 PNG 文件，不跟随符号链接 |
| 回答编造数字 | 数字溯源校验，重写一次仍不合格则降级为原始结果；不读任何表的 SQL（SELECT 常量拼出来的"结果"）被守卫拦下；问口径、表结构时不查数据直接回答，出处标明"未查询数据"，其中出现的数字也必须在表结构或口径里找得到 |
| 回答错了没人知道 | 每个回答可以点 👍/👎（差评可选原因）；运行记录同时是审计日志，`deepquery feedback-export` 把差评连同问题、上下文、SQL 导出成待标注的评测用例，补上标准答案就进回归集 |
| 查询"成功"但答案是错的 | COUNT / SUM 得 0 不会报错，最容易被当成真实答案：结果全为 0 或空、且过滤值在库里根本不存在时，按空结果处理，查出该列真实出现过的取值交给修复轮 |
| 成本失控 | 单次提问的 token 与金额预算熔断，修复轮次上限 |
| 公网演示被刷额度 | 访问口令、每访客限流、全站并发上限与每日花费上限；浏览器断开即停止调用模型，失败和中途取消的花费同样计入上限；访客的记忆按浏览器隔离（见 [部署指南](docs/DEPLOY.md)） |
